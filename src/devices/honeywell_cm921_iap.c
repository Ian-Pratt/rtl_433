...
static data_t *add_hex_string(data_t *data, const char *name, const uint8_t *buf, size_t buf_sz)
{
    if (buf && buf_sz > 0) {
        ...
        data = data_str(data, name, "", NULL, tstr);
    }
    return data;
}
...
static data_t *decode_device_ids(const message_t *msg, data_t *data, int style)
{
    ...
    return data_str(data, "ids", "Device IDs", NULL, ds);
}

#define UNKNOWN_IF(C) do { \
                        if (C) \
                           return data_int(data, "unknown", "", "%04x", msg->command); \
                      } while (0)
    ...
    switch (msg->command) {
        ...
        case 0x313F: {
            ...
            switch (msg->payload_length) {
                case 1:
                    data = data_int(data, "time_request", "", NULL, msg->payload[0]); break;
                case 9: {
        ...
        case 0x000a: {
            ...
            for (size_t i=0; i < msg->payload_length; i+=6) {
                ...
                data = data_int(data, name, "", "%02x", temp);
...
while( sep < bytes.bits_per_row[row])
{
    ...
    /* clang-format off */
    data_t *data = data_make( "model", "", DATA_STRING, "Honeywell-CM921-IAP", NULL );
    /* clang-format on */

    data = honeywell_cm921_interpret_message(decoder, &message, data);

    data = data_str(data, "mic", "Integrity", NULL, (pr>0) ? "CHECKSUM" : "CSUM_FAIL");
...
#ifdef _DEBUG
    if (pr<=0) data = data_int(data, "err", "Error", NULL, pr);
...
r_device const honeywell_cm921_iap = {
        .name        = "Honeywell CM921 Wireless Programmable Room Thermostat (IAP)",
        ...
        .disabled    = 1, // comparison decoder; enable explicitly with -R
};
